...
def sync_shopify():
    ...
    if shopify_settings.sync_every:

        import datetime
        now = frappe.utils.get_datetime()
        # now_plus_N = now + datetime.timedelta(minutes=shopify_settings.sync_every)
        next_sync = ""
        ...
        if sync:
            ...
            # now_time = frappe.utils.now() #DEBUG
...
def sync_shopify_resources():
    ...
    make_shopify_log(title="Sync Job Queued", status="Queued", method=frappe.local.form_dict.cmd,
    ...
    if shopify_settings.enable_shopify:
        ...
        make_shopify_log(
            ...
            message=_(
                """Shopify connector is not enabled. Click on 'Connect to Shopify' to connect ERPNext and your Shopify store."""),
            exception=True)
